...
def goals_summary(
    ...
    start_dt = datetime.combine(ps, datetime.min.time()).replace(tzinfo=timezone.utc)
    end_dt = datetime.combine(pe + timedelta(days=1), datetime.min.time()).replace(tzinfo=timezone.utc)
    ...
    for s in sess_rows:
        ...
        day_key = s.start_at.date().isoformat()
    ...
    if target_total > 0 and completed_total >= target_total:
        achievements.append({"key": "weekly_goal", "title": "Weekly goal achieved", "detail": "You hit your total target hours for this period."})
    if streak >= 3:
        achievements.append({"key": "streak", "title": f"{streak}-day streak", "detail": "Studied at least 15 minutes per day."})
    if completed_tasks >= 3:
        achievements.append({"key": "tasks", "title": "Deadline crusher", "detail": f"Completed {completed_tasks} deadlines this period."})
