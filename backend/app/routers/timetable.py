...
def _extract_items_grid(img: Image.Image) -> List[TimetableExtractItem]:
    ...
    # Pre-process to improve OCR on small/blurred screenshots (upscale + contrast/sharpen).
    # Note: We perform OCR on the processed image; coordinates below use the processed image size.
    ocr_img = img
    try:
        w0, h0 = img.size
        scale = 1
        ...
        from PIL import ImageEnhance, ImageOps  # local import to keep dependency optional
...
def _read_csv_text(raw: str) -> TimetableExtractResponse:
    ...
    def norm(h: str) -> str:
        # Normalize headers and lookup keys so formats like "Start Time" and "start_time" match.
        # We remove all non-alphanumerics (including underscores/spaces) to be robust.
