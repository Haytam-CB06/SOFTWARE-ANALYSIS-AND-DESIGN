...
export async function updateStudyTimetable(
  timetableId: string,
  userId: string,
  update: { name?: string; data?: Record<string, any>; is_active?: boolean }
): Promise<BackendStudyTimetable | null> {
  if (!isUuidLike(timetableId) || !isUuidLike(userId)) return null;
  const res = await fetch(`${baseUrl()}/study-timetables/${encodeURIComponent(timetableId)}`, {
    method: 'PUT',
...
export async function applyStudyTimetable(
  timetableId: string,
  userId: string,
  payload: { week_id: string; mode: 'overwrite' | 'merge'; tz_offset_minutes: number; activate?: boolean }
): Promise<{ ok: boolean; created: number; skipped_duplicates: number; skipped_overlaps: number } | null> {
  if (!isUuidLike(timetableId) || !isUuidLike(userId)) return null;
  const res = await fetch(`${baseUrl()}/study-timetables/${encodeURIComponent(timetableId)}/apply`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'X-User-Id': userId },
    body: JSON.stringify({
  ...
  return (await jsonOrThrow(res)) as any;
}
